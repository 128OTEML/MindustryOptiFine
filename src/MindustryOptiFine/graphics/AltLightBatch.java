...
import MindustryOptiFine.gen.LightsAtlas.*;
...
public class AltLightBatch extends SpriteBatch{
    ...
    void updateGlowAlpha(){
        // Get color from the packed color
        tmpColor.abgr8888(colorPacked);
        float r = tmpColor.r;
        float g = tmpColor.g;
        float b = tmpColor.b;
        float a = tmpColor.a;

        float lum = Mathf.curve((r + g + b) / 3f, 0.2f, 0.6f);
        float sat = Mathf.curve(Math.max(r, Math.max(g, b)) - Math.min(r, Math.min(g, b)), 0.1f, 0.3f);
        float v = Mathf.clamp(sat + lum);

        glowAlpha = Mathf.curve(a, 0.7f * (1f - v), 1f);
    }
    ...
    protected void draw(TextureRegion region, float x, float y, float originX, float originY, float width, float height, float rotation){
        if(flushing){
            ...
            TextureRegion rep;
            if((rep = MindustryOptiFine.shaderReplace.get(region)) != null){
                TextureRegion last = region;
                region = rep;
                if(MindustryOptiFine.autoGlowRegions.contains(region)) colorPacked = c;
        ...
        TextureRegion rep;
        if((rep = MindustryOptiFine.replace.get(region)) != null){
            TextureRegion last = region;
            region = rep;
        ...
        float color = (glow || MindustryOptiFine.autoGlowRegions.contains(region)) ? this.colorPacked : blackAlphaBits;
        float mixColor = this.mixColorPacked;

        if(!glow && (liquidMode || MindustryOptiFine.liquidRegions.contains(region))){
            tmpColor.abgr8888(colorPacked);
            float v = liquidGlow < 0 ? MindustryOptiFine.glowingLiquidColorsFunc.get(tmpColor) : liquidGlow;
            tmpColor.set(blackAlpha).lerp(tmpColor, v);
            color = tmpColor.toFloatBits();
        }

        if(manualGlow > -0.9f){
            tmpColor.abgr8888(colorPacked);
            color = tmpColor.set(tmpColor).mul(manualGlow).toFloatBits();
            rq.manual = true;
        }
        ...
        if(!glowTexture){
            ...
            boolean lg = glow;
            glow = true;
            if((gc = MindustryOptiFine.glowEquiv.get(region)) != null){
    ...
    protected void draw(Texture texture, float[] spriteVertices, int offset, int count){
        ...
        */
        ...
        if(manualGlow > -0.9f){
            tmpColor.abgr8888(colorPacked);
            float c = tmpColor.set(tmpColor).mul(manualGlow).toFloatBits();
            for(int i = 2; i < 24; i += 6){
                vertices[i] = c;
            }
            rq.color = c;
            ...
            float color = (glow || MindustryOptiFine.uvAutoGlowRegions.contains(ALStructs.uv(texture, vertices[3], vertices[4]))) ? colorPacked : blackAlphaBits;
            for(int i = 2; i < 24; i += 6){
                vertices[i] = color;
            }
        }
        ...
        if(!glowTexture){
            ...
            if((r = MindustryOptiFine.uvGlowRegions.get(ALStructs.uv(texture, su, sv))) != null){
                boolean lg = glow;
                glowTexture = true;
                glow = true;
    ...
    protected void setColor(Color tint){
        tmpColor.set(tint);
        setPackedColor(tmpColor.toFloatBits());
