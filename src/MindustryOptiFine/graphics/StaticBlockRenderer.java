...
import MindustryOptiFine.graphics.AltCacheSpriteBatch.*;
import mindustry.*;
...
public class StaticBlockRenderer{
    ...
    public void handleTile(Tile tile){
        ...
        EnviroGlow eg;
        if((eg = MindustryOptiFine.glowingEnvTiles.get(tile.block().id)) != null){
            if(!eg.floor){
                //glowingFloor.add(tile);
                glowing.add(tile);
            }
        }
        if((eg = MindustryOptiFine.glowingEnvTiles.get(tile.floor().id)) != null){
            if(eg.floor){
                glowingFloor.add(tile);
            }
        }
    ...
    void insertFloor(Tile tile){
        ...
        EnviroGlow g = MindustryOptiFine.glowingEnvTiles.get(tile.floor().id);
    ...
    void insertTile(Tile tile){
        ...
        if(!AltCacheSpriteBatch.glowing){
            ...
            EnviroGlow g = MindustryOptiFine.glowingEnvTiles.get(tile.block().id);
            drawGlowWall(tile, g);
        }
    }
